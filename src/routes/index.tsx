...
import { ArrowRight, ShoppingBag, Store, Bike, Users, Package, Truck, CreditCard, ShieldCheck, Search, ClipboardList, BarChart3, MapPin, Check, X } from "lucide-react";
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 ease-out">
            ...
            <p className="mt-8 max-w-2xl text-xl leading-relaxed text-white/80 md:text-2xl">
              GoLivra réunit commerces, clients et transporteurs sur une plateforme unique 
              pour transformer l'économie locale.
            </p>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-6 max-w-2xl text-lg text-muted-foreground">
              Nous avons repensé chaque étape du commerce pour créer une expérience 
              fluide et rentable pour tous les acteurs.
            </p>
          </div>
          ...
          <div className="mt-20 grid gap-8 md:grid-cols-3">
            {[
              ...
              <div key={f.title} className="group relative rounded-[2.5rem] border border-border bg-card p-10 transition-all hover:-translate-y-2 hover:border-primary/20 hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)]">
                <div className={`flex h-16 w-16 items-center justify-center rounded-2xl transition-transform group-hover:scale-110 ${f.color}`}>
                  <f.icon className="h-8 w-8" />
                </div>
                <h3 className="mt-8 text-2xl font-black">{f.title}</h3>
                <p className="mt-4 leading-relaxed text-muted-foreground">{f.text}</p>
                <Link to={f.to} className="mt-8 inline-flex items-center gap-2 text-sm font-black uppercase tracking-widest text-primary transition-all hover:gap-3">
                  En savoir plus
          ...
          <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-black tracking-tight md:text-6xl">
                Du clic à la porte,<br />en un temps record.
              </h2>
              <p className="mt-6 text-lg text-muted-foreground">
                GoLivra automatise la mise en relation entre commerçants, clients et livreurs 
                pour supprimer les frictions du commerce traditionnel.
              </p>
              
              <div className="mt-12 space-y-8">
                {[
                  { t: "Publication", d: "Les commerçants listent leurs produits en quelques secondes." },
      ...
      <section className="py-24 lg:py-32">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-20 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { icon: ShoppingBag, title: "Marketplace", text: "Visibilité instantanée pour vos produits." },
              { icon: ClipboardList, title: "Gestion Commandes", text: "Fini le chaos des messages WhatsApp." },
              { icon: Truck, title: "Réseau Logistique", text: "Livreurs certifiés à votre disposition." },
              { icon: CreditCard, title: "Transactions", text: "Paiements sécurisés et virements rapides." },
              { icon: BarChart3, title: "Analytique", text: "Comprenez vos ventes et vos clients." },
              { icon: ShieldCheck, title: "Confiance", text: "Modération et protection des acheteurs." },
            ].map((c) => (
              <article key={c.title} className="group rounded-3xl border border-border bg-card p-8 transition-all hover:bg-primary hover:border-primary">
                ...
                <p className="mt-4 leading-relaxed text-muted-foreground group-hover:text-white/80">{c.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="bg-foreground py-24 text-white lg:py-32">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <p className="mx-auto mt-8 max-w-2xl text-lg leading-relaxed text-white/60">
            Le commerce traditionnel à Brazzaville est plein de frictions. 
            GoLivra apporte la structure nécessaire pour passer à la vitesse supérieure.
          </p>
          ...
          <div className="mt-16 space-y-4 text-left max-w-2xl mx-auto">
            {[
              ...
              <div key={i} className="group flex items-center justify-between rounded-2xl bg-white/5 p-4 transition-colors hover:bg-white/10">
      ...
      <section className="relative py-24 lg:py-40 overflow-hidden">
        ...
        <div className="mx-auto max-w-5xl px-6 text-center">
          <h2 className="text-5xl font-black tracking-tighter md:text-8xl">
            Prêt à <span className="text-primary italic">propulser</span><br />votre activité ?
          </h2>
          <p className="mx-auto mt-8 max-w-2xl text-xl leading-relaxed text-muted-foreground md:text-2xl">
            Rejoignez des centaines de commerçants et transporteurs qui façonnent 
            le futur du commerce au Congo.
          </p>
          <div className="mt-12 flex flex-col items-center justify-center gap-6 sm:flex-row">
            <Link to="/solution" className="group flex w-full items-center justify-center gap-2 rounded-full bg-primary px-10 py-5 text-xl font-black text-primary-foreground shadow-2xl transition-all hover:scale-105 hover:bg-primary/90 active:scale-95 sm:w-auto">
              Commencer maintenant
              <ArrowRight className="h-6 w-6 transition-transform group-hover:translate-x-2" />
            </Link>
            <Link to="/" className="flex w-full items-center justify-center gap-2 rounded-full border border-border bg-card px-10 py-5 text-xl font-black transition-all hover:bg-secondary active:scale-95 sm:w-auto">
              Contacter l'équipe
            </Link>
          </div>
